...
pub async fn get_inference(
    ...
    println!("Fetching ollama generation from {}", &url);
    ...
    let response = client
        ...
        .await
        .map_err(|err| {
            let err_str = format!("Inference Request failed with status: {}", err);
            Error::StringError(err_str)
        })?;
    println!("+++++Response: ");
    let res = response.text().await.unwrap();
    // Parse the response text as JSON
    let json_res: Value = serde_json::from_str(&res).map_err(|err| {
        let err_str = format!("Failed to parse JSON: {}", err);
        Error::StringError(err_str)
    })?;
    dbg!(&json_res);
